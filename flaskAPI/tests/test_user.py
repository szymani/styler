...
class TestUser:
    ...
    def test_get_user_self(self, client, app, login, password, email, status_code):
        ...
        assert (response.status_code == 200) and (
            response.get_json() is not None), response.get_json()
    ...
    def test_update_user_self(self, client, app, login, password, email, login2, password2, email2, status_code):
        ...
        url = '/user'
        response = client.put(
            url, json=data, headers=headers, follow_redirects=True)
        assert (response.status_code == 200) and (
            response.get_json() is not None), response.get_json()
    ...
    def test_get_user_by_id(self, client, login, password, user_type, status_code):
        first_user = user_model.User(
            login=login, password=password, email="admin_email", user_type=user_type)
        second_user = user_model.User(
            login="someLogin", password="somePassword", email="someEmail", user_type=0)
    ...
    def test_update_user_by_id(self, client, app, login, password, email, login2, password2, email2, user_type, status_code):
        first_user = user_model.User(
            login=login, password=password, email=email, user_type=user_type)
        second_user = user_model.User(
            login="someLogin", password="somePassword", email="someEmail", user_type=0)
        db.session.add(first_user, second_user)
        db.session.commit()
